...
public class FdrConversionBlobTrigger {
    ...
    public boolean process (
        ...
        log.info("[{}] Triggered, id = {}, blob-name = {}, blob-metadata = {}, retry = {}", FN_NAME, iid, blobName, blobMetadata, retryIndex);
        ...
        if (!Boolean.parseBoolean(blobMetadata.getOrDefault(ELABORATE_KEY, "false"))) {
            log.info("[{}] Skipped, id = {}, blob-name = {}, blob-metadata = {}, retry = {}", FN_NAME, iid, blobName, blobMetadata, retryIndex);
            return false;
        }
        ...
        try {
            ...
            log.info("[{}][id={}] Successful conversion call to FdR1", FN_NAME, context.getInvocationId());
        }
        catch (Exception e) {
            log.error("[Exception][{}][id={}] class = {}, message = {}", FN_NAME, iid, e.getClass(), e.getMessage());
            // Get error type
    ...
    private static void sendToDeadLetter(ExecutionContext context, String blob, Map<String, String> metadata, String message, ErrorEnum errorEnum, String response, Exception e) {
        log.warn("[ALERT][{}][LAST_RETRY][DEAD-LETTER] Performed last retry for event ingestion: InvocationId [{}]", FN_NAME, context.getInvocationId());
